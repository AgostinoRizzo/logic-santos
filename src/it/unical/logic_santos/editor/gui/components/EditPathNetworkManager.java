 ...
 *
 */
public class EditPathNetworkManager implements ActionListener {
	...
	private JLabel lblOperation = new JLabel("Walking Paths Operation: ");
	...
	public EditPathNetworkManager(LogicSantosEditorApplication app, JFrame _mainWindow, JPanel _toolsPanel) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().compareTo("Edit Paths Network") == 0) {
			loadToolsComponents();
			...
			application.getCanvasEditor().getPathsNetwork().deselectPreviousNode();
		} else if (e.getSource().equals(cbUsePreviousNodeSelection)) {
			application.getCanvasEditor().getPathsNetwork().setUsePreviousNode(cbUsePreviousNodeSelection.isSelected());
		}
	}
	...
	private void loadToolsComponents() {
		...
		//this.toolsPanel.updateUI();
		//this.toolsPanel.repaint();
		application.setEditorStatus(EditorStatus.ADD_PATHS_NETWORK_NODE);
	}
	...
	private void removeRoadNetworkNode() {
		application.setEditorStatus(EditorStatus.REMOVE_PATHS_NETWORK_NODE);
	}
	
	private void modifyRoadNetworkNode() {
		application.setEditorStatus(EditorStatus.MODIFY_PATHS_NETWORK_NODE);
	}
	
	private void addRoadNetworkArc() {
		application.setEditorStatus(EditorStatus.ADD_PATHS_NETWORK_ARC);
	}

	private void removeRoadNetworkArc() {
		application.setEditorStatus(EditorStatus.REMOVE_PATHS_NETWORK_ARC);
	}
